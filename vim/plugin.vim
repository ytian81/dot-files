...
let g:fzf_layout = { 'window': { 'width': 0.8, 'height': 0.6, 'highlight': 'Comment', 'rounded': v:false } }
let g:fzf_commits_log_options = '--graph --color=always --format="%C(auto)%h %C(green)%C(bold)%an%C(auto): %s%d %C(magenta)%cr"'
" let g:fzf_colors =
"     \ { 'fg':      ['fg', 'Normal'],
"       \ 'bg':      ['bg', 'Normal'],
"       \ 'hl':      ['fg', 'Comment'],
